/*
  ...
  Change the #define _ASSIGNED_PORT_ to any other port you wish to use
   ...
   make [clean] [linux] [debug_linux] [freebsd] [debug_freebsd]
  ...
  Updated to v0.13 - Code modernization and cleanup
...
struct alarm_signal as;
struct radio_report rpt;
struct radio_command cmd;
...
void report_reply_data_socket(int socket) {
  ...
  strncpy( signal, ( (char *)&rpt.sig + 2 ), 3 );

  /* Now do the frequency offset */
  if ( strcmp( rpt.foffset, "I280" ) == 0 ) {
    sprintf( offset, "Centered" );
  }
  else if ( strcmp( rpt.foffset, "I200" ) == 0 ) {
    sprintf( offset, "Low" );
  }
  else if ( strcmp( rpt.foffset, "I2FF" ) == 0 ) {
    sprintf( offset, "High" );
  }
  else {
    sprintf( offset, "Unknown" );
  }

  /* Now, do the squelch state */
  if ( strcmp( rpt.squ, "I004" ) == 0 ) {
    sprintf( squelch, "Closed " );
  }
  else if ( strcmp( rpt.squ, "I007" ) == 0 ) {
    sprintf( squelch, "Open " );
  }
  else {
    sprintf( squelch, "Unknown" );
  }
...
void report_reply_data( void ) {
  ...
  if ( ( rpt.report_enable == 1 )
    ...
    convert = rpt.last_mode;
    switch ( convert ) {
    case 0:
      sprintf( mode_str, "LSB" );
      break;
    case 1:
      sprintf( mode_str, "USB" );
      break;
    case 2:
      sprintf( mode_str, "AM" );
      break;
    ...
    case 4:
      ...
      break;
    case 5:
      sprintf( mode_str, "NFM" );
      break;
    case 6:
      sprintf( mode_str, "WFM" );
      break;
    default:
      sprintf( mode_str, "Unknown" );
      break;
    ...
    /* Convert filter integer to string */
    convert = rpt.last_filter;
    switch ( convert ) {
    case 0:
      sprintf( filter_str, "2.8K" );
      break;
    case 1:
      sprintf( filter_str, "6K" );
      break;
    case 2:
      sprintf( filter_str, "15K" );
      break;
    case 3:
      sprintf( filter_str, "50K" );
      break;
    case 4:
      sprintf( filter_str, "230K" );
      break;
    default:
      sprintf( filter_str, "Unknown" );
      break;
    }
